...
- **TDD**: All test tasks marked with 🔴 (RED) must be written FIRST and FAIL before implementation
...
**Human Review Checkpoint 🛑**: After Phase 1, pause for review and approval before proceeding to Phase 2

- [ ] T001 Verify Node.js 18+ installed and npm available in development environment
- [ ] T002 [P] Initialize Docusaurus 3.x project using `npx create-docusaurus@latest physical-ai-book classic --typescript false`
- [ ] T003 [P] Create GitHub repository and push initial Docusaurus scaffold
- [ ] T004 Configure `docusaurus.config.js` with site metadata (title: "Physical AI Book", description, GitHub Pages URL, organizational metadata per ADR-001)
- [ ] T005 [P] Configure Git ignore for `node_modules/`, `.docusaurus/`, `build/`, `.DS_Store`
- [ ] T006 [P] Create README.md with setup instructions (Node.js requirements, installation, local development, deployment)
- [ ] T007 Verify local development server runs without errors: `npm run start`
- [ ] T008 Verify static build completes without errors: `npm run build`
...
**Human Review Checkpoint 🛑**: After Phase 2, pause for review and approval before proceeding to Phase 3

### Configuration Foundation

- [ ] T009 Define frontmatter schema in CONTRIBUTING.md documentation (required fields: title, description, difficulty, chapter, chapter_number; optional fields: learning_objectives, prerequisites, estimated_time, keywords per ADR-003)
- [ ] T010 Create docs/ directory structure: `docs/intro.md`, `docs/chapter-01-foundations/`, `docs/chapter-02-mechanics/`, `docs/glossary.md` per ADR-003
- [ ] T011 Configure `sidebars.js` with autogenerated sidebar structure for chapter-01-foundations and chapter-02-mechanics per ADR-003
- [ ] T012 [P] Configure Prism syntax highlighting in `docusaurus.config.js` for Python, JavaScript, C++, Java, Rust, Go languages (FR-008)
- [ ] T013 [P] Configure search plugin in `docusaurus.config.js` (use Docusaurus built-in search per ADR-001)

### Test Infrastructure Setup (TDD Foundation)

- [ ] T014 Install Playwright as dev dependency: `npm install --save-dev @playwright/test`
- [ ] T015 [P] Install axe-core for accessibility testing: `npm install --save-dev @axe-core/playwright`
- [ ] T016 [P] Install Lighthouse CI: `npm install --save-dev @lhci/cli`
- [ ] T017 Create `playwright.config.js` with browser configurations (Chromium, Firefox, WebKit) and base URL settings
- [ ] T018 Create `.lighthouserc.js` with performance budgets (score ≥90, load time <3s per SC-006)
- [ ] T019 Create `tests/` directory structure: `tests/navigation.test.js`, `tests/search.test.js`, `tests/accessibility.test.js`, `tests/links.test.js`, `tests/responsive.test.js`
- [ ] T020 [P] Create test helpers in `tests/helpers/test-utils.js` (page load utilities, common assertions, viewport configurations)

**Checkpoint**: All configuration and test infrastructure complete - ready for user story implementation

---

## Phase 3: User Story 1 - Browse Physical AI Book Content (Priority: P1) 🎯 MVP

**Goal**: Readers can browse book content through intuitive navigation with previous/next links and sidebar table of contents

**Independent Test**: Deploy site with sample content; verify readers can navigate homepage → chapters via sidebar and use prev/next navigation

**Duration Estimate**: 3-5 days

**Human Review Checkpoint 🛑**: After Phase 3, pause for MVP demo/review and approval before proceeding to Phase 4

### 🔴 RED: Tests for User Story 1 (Write FIRST, Ensure FAIL)

**TDD Workflow**: Write these tests, run them, verify they FAIL, then implement features

- [ ] T021 🔴 [P] [US1] Write navigation structure test in `tests/navigation.test.js`: verify sidebar displays, chapter links clickable, breadcrumbs present (should FAIL - no content yet)
- [ ] T022 🔴 [P] [US1] Write prev/next navigation test in `tests/navigation.test.js`: verify previous/next buttons exist on chapter pages and navigate correctly (should FAIL - no content yet)
- [ ] T023 🔴 [P] [US1] Write homepage test in `tests/navigation.test.js`: verify homepage loads, displays book title, shows introduction, provides entry points to chapters (should FAIL - no homepage content yet)
- [ ] T024 🔴 [P] [US1] Write content rendering test in `tests/navigation.test.js`: verify markdown renders (headings, lists, tables, images, code blocks) (should FAIL - no content yet)

**Checkpoint**: All US1 tests written and FAILING - ready for implementation

### 🟢 GREEN: Implementation for User Story 1

**Context7/MCP Integration Point** 📚: Use context7 MCP server to look up Docusaurus frontmatter documentation and sidebar configuration best practices

- [ ] T025 [P] [US1] Create `docs/intro.md` with book introduction content and frontmatter (title, description, difficulty: beginner)
- [ ] T026 [P] [US1] Create sample chapter `docs/chapter-01-foundations/index.md` with chapter overview and frontmatter (chapter: "Foundations", chapter_number: 1, learning_objectives)
- [ ] T027 [P] [US1] Create sample sections in chapter-01: `docs/chapter-01-foundations/01-introduction.md`, `docs/chapter-01-foundations/02-history.md`, `docs/chapter-01-foundations/03-key-concepts.md` with full frontmatter per schema
- [ ] T028 [P] [US1] Create sample chapter `docs/chapter-02-mechanics/index.md` and 3 sample sections with frontmatter
- [ ] T029 [US1] Update `sidebars.js` to configure sidebar navigation tree with auto-generated chapter sections per ADR-003 (depends on T026-T028)
- [ ] T030 [US1] Configure homepage in `docusaurus.config.js` to use `docs/intro.md` as landing page (FR-010)
- [ ] T031 [US1] Verify previous/next navigation auto-generates from sidebar configuration (FR-004)
- [ ] T032 [US1] Add custom CSS in `src/css/custom.css` for responsive typography (minimum 16px body font, readable line height per SC-004)

**Checkpoint**: Run tests - all US1 navigation tests should now PASS

### ♻️ REFACTOR: Clean up User Story 1

- [ ] T033 [US1] Review and optimize sidebar configuration (remove duplication, ensure clear hierarchy)
- [ ] T034 [US1] Standardize frontmatter across all sample content files (validate all required fields present)
- [ ] T035 [US1] Test manual navigation flow: homepage → chapter in 2 clicks (validate SC-007)
- [ ] T036 [US1] Verify site loads locally without errors and content is readable on desktop and mobile
...
**Human Review Checkpoint 🛑**: After Phase 4, pause for beta demo/review and approval before proceeding to Phase 5

### 🔴 RED: Tests for User Story 2 (Write FIRST, Ensure FAIL)

- [ ] T037 🔴 [P] [US2] Write search functionality test in `tests/search.test.js`: verify search box displays in navigation bar (should FAIL - search not configured yet)
- [ ] T038 🔴 [P] [US2] Write search query test in `tests/search.test.js`: verify search returns results for sample queries (e.g., "Physical AI", "robotics") within 1 second (should FAIL - search not functional yet)
- [ ] T039 🔴 [P] [US2] Write search result navigation test in `tests/search.test.js`: verify clicking search results navigates to correct page with term highlighting (should FAIL - search not implemented)
- [ ] T040 🔴 [P] [US2] Write "no results" state test in `tests/search.test.js`: verify search displays helpful message for non-existent queries (should FAIL - no error handling yet)

**Checkpoint**: All US2 tests written and FAILING - ready for implementation

### 🟢 GREEN: Implementation for User Story 2

**Context7/MCP Integration Point** 📚: Look up Docusaurus search plugin configuration and client-side search indexing documentation

- [ ] T041 [US2] Enable Docusaurus built-in search plugin in `docusaurus.config.js` (configure search index settings per ADR-001)
- [ ] T042 [US2] Configure search UI in theme settings (ensure search box visible in navigation bar per FR-003)
- [ ] T043 [US2] Test search indexing by rebuilding site and verifying content is indexed: `npm run build`
- [ ] T044 [US2] Verify search returns results for various keywords (test with content from chapter-01, chapter-02)
- [ ] T045 [US2] Configure search result highlighting (ensure search terms highlighted in results and target pages)
- [ ] T046 [US2] Add "no results" state handling in search configuration (display helpful message for empty results)

**Checkpoint**: Run tests - all US2 search tests should now PASS

### ♻️ REFACTOR: Clean up User Story 2

- [ ] T047 [US2] Optimize search configuration (tune relevance, result limits, indexing rules)
- [ ] T048 [US2] Test search performance: verify p95 response time <1 second for 10 sample queries (validate SC-002)
- [ ] T049 [US2] Test edge cases: special characters in queries, code syntax searches, non-English characters
- [ ] T050 [US2] Verify search works correctly after new content added (test search re-indexing after content update)
...
**Human Review Checkpoint 🛑**: After Phase 5, pause for CI/CD review and approval before proceeding to Phase 6

### 🔴 RED: Tests for User Story 3 (Write FIRST, Ensure FAIL)

- [ ] T051 🔴 [US3] Write link validation test in `tests/links.test.js`: verify all internal links resolve correctly (100% pass rate per SC-005) (should FAIL - no validation script yet)
- [ ] T052 🔴 [US3] Write accessibility test in `tests/accessibility.test.js`: run axe-core on homepage, chapter pages, search page; verify zero violations (should FAIL - axe-core not integrated)
- [ ] T053 🔴 [US3] Write keyboard navigation test in `tests/accessibility.test.js`: verify Tab, Enter, Arrow keys navigate site elements (should FAIL - not tested yet)
- [ ] T054 🔴 [US3] Write deployment smoke test in `tests/navigation.test.js`: verify deployed site is reachable and homepage loads (should FAIL - no deployment yet)

**Checkpoint**: All US3 tests written and FAILING - ready for CI/CD implementation

### 🟢 GREEN: Implementation for User Story 3

**Context7/MCP Integration Point** 📚: Look up GitHub Actions workflows for Docusaurus deployment to GitHub Pages and Playwright CI integration

- [ ] T055 [US3] Create GitHub Actions workflow file `.github/workflows/deploy.yml` with basic structure (checkout, setup Node.js 18+, install dependencies per ADR-002)
- [ ] T056 [US3] Add link validation step to workflow: install markdown-link-check or similar; check all docs/ files for broken links (FR-018)
- [ ] T057 [US3] Add Playwright test execution step to workflow: run navigation, search, responsive tests (FR-015)
- [ ] T058 [US3] Add axe-core accessibility test execution step to workflow: test homepage + 3 representative chapter pages (FR-016, FR-017 per ADR-002)
- [ ] T059 [US3] Add Lighthouse CI step to workflow: test performance score ≥90 on homepage and sample chapters (SC-006 per ADR-002)
- [ ] T060 [US3] Add Docusaurus build step to workflow: `npm run build` (generate static site)
- [ ] T061 [US3] Add GitHub Pages deployment step to workflow: deploy to gh-pages branch (only if all tests pass per FR-014)
- [ ] T062 [US3] Configure workflow to fail deployment if any test fails (keep previous version live per ADR-002)
- [ ] T063 [US3] Configure GitHub repository settings: enable GitHub Pages on gh-pages branch with HTTPS enforced (FR-007)

**Checkpoint**: Run workflow - verify CI/CD pipeline runs and deploys site to GitHub Pages

### ♻️ REFACTOR: Clean up User Story 3

- [ ] T064 [US3] Optimize CI workflow: parallelize independent test steps (link validation + accessibility tests can run concurrently)
- [ ] T065 [US3] Add workflow status badge to README.md (show build passing/failing status)
...
- [ ] T068 [US3] Document CI/CD workflow in CONTRIBUTING.md (explain test gates, deployment process, rollback behavior)
...
**Human Review Checkpoint 🛑**: After Phase 6, pause for responsive testing review and approval before proceeding to Phase 7
...
**Human Review Checkpoint 🛑**: After Phase 7, pause for final v1.0 review and approval before production launch
...
**Context7/MCP Integration Point** 📚: Look up Docusaurus build performance optimization for large sites

- [ ] T104 Create 500 pages of sample content to test scale (50 chapters × 10 sections per SC-010)
...
- **Phase 3: User Story 1 (P1)** → Depends on Phase 2 complete - MVP target
- **Phase 4: User Story 2 (P2)** → Depends on Phase 2 complete (can run parallel to Phase 3 if staffed)
- **Phase 5: User Story 3 (P2)** → Depends on Phase 2 complete AND Phase 3 complete (needs content for deployment testing)
- **Phase 6: User Story 4 (P3)** → Depends on Phase 2 complete (can run parallel to other stories)
- **Phase 7: Polish** → Depends on ALL user stories complete

### Critical Path (Sequential)

1. **Setup (Phase 1)** → Initialize project
2. **Foundational (Phase 2)** → Configure infrastructure (BLOCKS all stories)
3. **User Story 1 (Phase 3)** → MVP content and navigation
4. **User Story 3 (Phase 5)** → Deployment (depends on US1 content)
5. **Polish (Phase 7)** → Production hardening
...
1. 🔴 **RED**: Write tests, run tests, verify FAIL
2. 🟢 **GREEN**: Implement minimal code to pass tests
3. ♻️ **REFACTOR**: Clean up code while keeping tests passing

**Example for User Story 1**:
1. Write T021-T024 (navigation tests) → Run → FAIL ✅
2. Implement T025-T032 (content and navigation) → Run tests → PASS ✅
3. Execute T033-T036 (refactor and manual validation) → Tests still PASS ✅
...
- PHR (Prompt History Record) links tasks → spec → plan → ADRs
...
**Phase 1 Complete 🛑**: Review project setup, verify Node.js and Docusaurus initialized correctly
- **Approval Required**: Confirm foundation ready before proceeding to Phase 2

**Phase 2 Complete 🛑**: Review configuration, verify test infrastructure set up correctly
- **Approval Required**: Confirm all foundational work complete before user story implementation

**Phase 3 Complete (MVP) 🛑**: Demo User Story 1 - readers can browse content with navigation
- **Approval Required**: Validate MVP meets acceptance criteria before adding search

**Phase 4 Complete 🛑**: Demo User Story 2 - readers can search book content
- **Approval Required**: Validate search functionality before proceeding to deployment

**Phase 5 Complete 🛑**: Review CI/CD pipeline, verify automated deployment working
- **Approval Required**: Confirm deployment quality gates working before responsive work

**Phase 6 Complete 🛑**: Demo responsive design on multiple devices
- **Approval Required**: Validate responsive experience before production hardening

**Phase 7 Complete (v1.0) 🛑**: Final review of production-ready site
...
- **SC-001**: Site loads in <3s on 50Mbps → Test with WebPageTest (T088)
- **SC-002**: Search returns results in <1s (p95) → Test with Playwright performance timing (T048)
- **SC-003**: Content updates live within 5 minutes → Test full deployment cycle (T066)
- **SC-004**: No horizontal scroll 320px-2560px + 16px font → Test with Playwright viewports (T077, T073)
- **SC-005**: 100% internal links resolve → Automated link checker in CI (T056)
- **SC-006**: Lighthouse score ≥90 → Lighthouse CI in workflow (T059, T087)
- **SC-007**: Navigate homepage → chapter in ≤2 clicks → Manual test (T035)
- **SC-008**: WCAG 2.1 AA zero violations → axe-core tests + manual audit (T058, T090-T095)
- **SC-009**: Build success rate ≥99% over 30 days → Monitor CI workflow (T113)
- **SC-010**: 500 pages without degradation → Scale test (T104-T106)
...
|-----------|-------------|------------------|--------------|
| **I. Content-First Documentation** | Markdown, frontmatter, progressive complexity | T009, T025-T028 (frontmatter schema), T010-T011 (chapter organization) | Verify docs/ structure follows progressive complexity (chapter-01-foundations → chapter-02-mechanics) |
...
| **VII. Test-First for Critical Paths** | Tests before implementation, red-green-refactor | T021-T024, T037-T040, T051-T054, T069-T073 (all 🔴 RED tests written FIRST) | Verify all test tasks marked 🔴 complete before implementation tasks |

**Constitutional Compliance Status**: ✅ **PASS** - All principles addressed in task breakdown

---

## Notes

- **TDD Discipline**: All test tasks marked 🔴 MUST be written FIRST and FAIL before implementation
...
- **Human Review**: 🛑 checkpoints after each phase ensure quality and alignment before proceeding
- **Context7**: 📚 markers indicate where to use MCP server for documentation lookups
...
**Task Breakdown Status**: ✅ COMPLETE - Ready for implementation (120 tasks, 7 phases, 4 user stories, TDD workflow)
