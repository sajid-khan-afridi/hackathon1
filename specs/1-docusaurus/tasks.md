...
- [X] T018 Create `.lighthouserc.js` with performance budgets (score ≥90, load time <3s per SC-006)
...
- [X] T021 🔴 [P] [US1] Write navigation structure test in `tests/navigation.test.js`: verify sidebar displays, chapter links clickable, breadcrumbs present (should FAIL - no content yet)
- [X] T022 🔴 [P] [US1] Write prev/next navigation test in `tests/navigation.test.js`: verify previous/next buttons exist on chapter pages and navigate correctly (should FAIL - no content yet)
- [X] T023 🔴 [P] [US1] Write homepage test in `tests/navigation.test.js`: verify homepage loads, displays book title, shows introduction, provides entry points to chapters (should FAIL - no homepage content yet)
- [X] T024 🔴 [P] [US1] Write content rendering test in `tests/navigation.test.js`: verify markdown renders (headings, lists, tables, images, code blocks) (should FAIL - no content yet)
...
- [X] T035 [US1] Test manual navigation flow: homepage → chapter in 2 clicks (validate SC-007)
...
- [X] T037 🔴 [P] [US2] Write search functionality test in `tests/search.test.js`: verify search box displays in navigation bar (should FAIL - search not configured yet)
- [X] T038 🔴 [P] [US2] Write search query test in `tests/search.test.js`: verify search returns results for sample queries (e.g., "Physical AI", "robotics") within 1 second (should FAIL - search not functional yet)
- [X] T039 🔴 [P] [US2] Write search result navigation test in `tests/search.test.js`: verify clicking search results navigates to correct page with term highlighting (should FAIL - search not implemented)
- [X] T040 🔴 [P] [US2] Write "no results" state test in `tests/search.test.js`: verify search displays helpful message for non-existent queries (should FAIL - no error handling yet)
...
- [x] T051 🔴 [US3] Write link validation test in `tests/links.test.js`: verify all internal links resolve correctly (100% pass rate per SC-005) (should FAIL - no validation script yet)
- [x] T052 🔴 [US3] Write accessibility test in `tests/accessibility.test.js`: run axe-core on homepage, chapter pages, search page; verify zero violations (should FAIL - axe-core not integrated)
- [x] T053 🔴 [US3] Write keyboard navigation test in `tests/accessibility.test.js`: verify Tab, Enter, Arrow keys navigate site elements (should FAIL - not tested yet)
- [x] T054 🔴 [US3] Write deployment smoke test in `tests/navigation.test.js`: verify deployed site is reachable and homepage loads (should FAIL - no deployment yet)
...
- [x] T059 [US3] Add Lighthouse CI step to workflow: test performance score ≥90 on homepage and sample chapters (SC-006 per ADR-002)
